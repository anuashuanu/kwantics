...
def Integer_Word(Number):
    Number = int(Number)
    LowDigit = ["", "एक", "दो", "तीन", "चार", "पाँच", "छह", "सात", "आठ", "नौ", "दस", "ग्यारह", "बारह", "तेरह",
                "चौदह",
                ...
                "छियासठ", "सड़सठ", "अड़सठ", "उनहत्तर", "सत्तर", "इकहत्तर", "बहत्तर", "तिहत्तर", "चौहत्तर",
                "पचहत्तर",
                "छिहत्तर", "सतहत्तर", "अठहत्तर", "उनासी", "अस्सी", "इक्यासी", "बयासी", "तिरासी", "चौरासी", "पचासी",
                "छियासी", "सत्तासी", "अट्ठासी", "नवासी", "नब्बे", "इक्यानबे", "बानबे", "तिरानबे", "चौरानबे",
                "पंचानबे",
    ...
    Word = (CroreWord + ' ' + LakhWord + ' ' + ThousandWord + ' ' + HundredWord + ' ' + TenthWord).strip()
    return Word
...
def LineWithNumber(a):
    p=[float(s) for s in re.findall(r'\d+\.?\d*',a)]
    p=sorted(p, reverse=True)
    print(p)
    for i in p:
        Word = Num_to_word(i)
        if (i).is_integer():
            a = str.replace(a,str(int(i)),Word)
        else:
            a = str.replace(a,str(i),Word)
    return a
...
def AddString():
    ...
    for File in AllFileList:
        ...
        text_file = open(fileNameWithPath, "r", encoding="utf8", errors='ignore')
        sentence = text_file.read()
        cleanSentence = cleanText(sentence)
        cleanSentence = LineWithNumber(cleanSentence)

        with open(fileNameWithPath, "w", encoding="utf8") as output:
            output.write(StartString)
            #Add starting tag <s>
            output.write(cleanSentence)
            #Add cleanSentence data
            output.write(EndString)
            # Add Ending tag </s> with wave file path
        output.close()
